...
function image_upload($img){
    ...
    if(!move_uploaded_file($tmp_loc,$new_loc)){
        header("location:addProducts.php?alert=img_upload");
        exit;
    }
    else{
       return $new_name;
...
function image_remove($img){
   if(!unlink(UPLOAD_SRC.$img)){
    header("location: addProducts.php?alert=img_rem_fail");
    exit;
   }
}
...
if(isset($_POST['addproduct']))
{
   ...
   $query="INSERT INTO `products`( `name`, `price`, `description`, `image`) 
    ...
    if(mysqli_query($con,$query)){
        header("location: addProducts.php?success=added");
    } 
    else{
        header("location: addProducts.php?alert=add_failed");
    }       
...
if(isset($_GET['rem']) && $_GET['rem']>0){
    ...
    if(mysqli_query($con, $query)){
        header("location: addProducts.php?success=removed");
    }else{
...
if(isset($_POST['editproduct'])){
    ...
    if(mysqli_query($con, $update)){
        header("location: addProducts.php?success=updated");
    }else{
        header("location: addProducts.php?alert=update_failed");
    }
}
